...
void init()
{
    // objasniti u izvestaju
    PIR1 = 0b00000000; // flegovi prijema preko UART-a
    PIE1 = 0b00100001; // dozvola prekida za UART, RCIE, TMR1IE
    // PIE1.TMR1IE = 1;
    // PIE1.RC1IE=1;

    // sta se ovde konkretno podesava preko T1CON?
    // pogledati datasheet PIC18F2525 i objasniti detaljno u izvestaju
    ...
    // sta se ovde konkretno podesava preko ADCONx?
    // pogledati datasheet PIC18F2525 i objasniti detaljno u izvestaju
...
{ // prekidi serijske komunikacije i tajmer1
    ...
    if ((PIE1.TMR1IE) && (PIR1.TMR1IF))
    {
        ...
        { // na svakih  125ms se proziva po jedna prostorija
            br = 0x00;
            Flag1 = 0x01; // podigo se flag koji govori da je doslo vreme da se prozove slave(prostorija)
        }
        else
            br++;
